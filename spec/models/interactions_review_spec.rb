...
RSpec.describe InteractionsReview, type: :model do
  describe "columns" do
    ...
    it { is_expected.to have_db_column(:classification_note).of_type(:integer).with_options(null: true, default: nil) }
    it { is_expected.to have_db_column(:disable_interactions).of_type(:boolean).with_options(null: false, default: true) }
  end
